...
export class StatusComponent implements OnInit{
	...
	importaStatus() {
		if(this.IPC.isElectron()) {
			...
			const button = document.getElementById('cargaStatus') as HTMLInputElement;
			if(button.files) {
				const reader = new FileReader();
				reader.readAsArrayBuffer(button.files[0]);
				reader.onload = () => {
					const decoder = new TextDecoder('utf-8');
					const res:string = decoder.decode(reader.result as ArrayBuffer);
	...
	cargaStatus (data:string) {
		...
		const arrayStatus:string[] = data.split('\n');
		arrayStatus.forEach((line:string) => {
			if(line.length > 1) {
				const datosClienteString:string = line.substring(line.indexOf('{')+1, line.indexOf('}'));
				const datos:string = line.substring(0,line.indexOf('{')-1)+line.substring(line.indexOf('}')+1);
				const arrayDatos:string[] = datos.split(',');
				const arrayDatosClientes:string[] = datosClienteString.split(';');
				const status:Status = new Status(parseInt(arrayDatos[0]),arrayDatos[1],arrayDatosClientes,arrayDatos[2],arrayDatos[3]);
	...
	buscaStatus() {
		const buscado = (document.getElementById('buscaStatus') as HTMLInputElement).value.toLowerCase();
		this.status = [];
